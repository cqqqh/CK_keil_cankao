...
char * itoa(UINT value, char * buf, UCHAR radix)// int to asc码
{
	UINT i;
	char * ptr;
	char * temphold;
   ...
   if ((request == GET_PAGE) || (request == POST_PAGE))
   {
     ...
     // if (outbuf == NULL)
      ...
      // Replace length tag with actual value
		...
		// Replace CPU temperature tag with actual value   重置CPU温度为实际值		 （需要修改的地方）
		...
		// Replace CPU voltage tag with actual value X 100	  重置CPU电压基准为实际电压的100倍
		// Insert decimal point between first and second digits	  在第一和第二个数之间插入小数点	   （需要修改的地方）
